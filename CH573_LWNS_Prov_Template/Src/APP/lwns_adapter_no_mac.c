...
static u8 ble_phy_manage_state; //phy状态管理
...
static void RF_2G4StatusCallBack(uint8 sta, uint8 crc, uint8 *rxBuf) {
    ...
    case TX_MODE_TX_FAIL:
        ...
        tmos_set_event(lwns_phyoutput_taskid, LWNS_PHY_OUTPUT_FINISH_EVT);
        break;
    default:
        break;
    }
...
#if ROUTE_ENTRY_MANUAL_NUM
    cfg.routetable_ptr = route_entry_memp; //如果需要使用mesh，必须分配。不然会进入hardfault。
#else
...
static uint16 lwns_phyoutput_ProcessEvent(uint8 task_id, uint16 events) {
    ...
    if (events & SYS_EVENT_MSG) {
        ...
        if ((pMsg = tmos_msg_receive(lwns_phyoutput_taskid)) != NULL) {
            ...
            RF_Shut();
            RF_Tx((u8 *) (pMsg + 1), pMsg[0], USER_RF_RX_TX_TYPE, USER_RF_RX_TX_TYPE);
